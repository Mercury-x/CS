...
class Ant(Insect):
    ...
    def __init__(self, health=1):
        ...
        self.doubled = False  # is damage being doubled
...
class WallAnt(Ant):
    """
    WallAnt do nothing in a turn, but have a laege health value
    """
    name = 'Wall'
    damage = 0
    food_cost = 4
    ...
    def __init__(self, health=4):
        super(WallAnt, self).__init__(health)
...
class HungryAnt(Ant):
    ...
    def __init__(self, health=1):
        super(HungryAnt, self).__init__(health)
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8a
        "*** YOUR CODE HERE ***"
        return self.ant_contained is None and other is not None and not other.is_container
        # END Problem 8a
...
class BodyguardAnt(ContainerAnt):
    ...
    def __init__(self, health=2):
        super(BodyguardAnt, self).__init__(health)
    # END Problem 8c
...
class TankAnt(ContainerAnt):
    ...
    def __init__(self, health=2):
        super(TankAnt, self).__init__(health)
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        # BEGIN Problem 10
        "*** YOUR CODE HERE ***"
        if insect is not None and insect.is_waterproof:
            Place.add_insect(self, insect)
        elif insect is not None:
...
def slow_action(func):
    def helper(self, gamestate):
        if self.turns_left > 0:
            self.turns_left -= 1
            print("DEBUG", "TIME: ", gamestate.time)
            if gamestate.time % 2 == 1:
                return

        return func(self, gamestate)
...
class Bee(Insect):
    ...
    def __init__(self, health=1):
        super(Bee, self).__init__(health)
        self.turns_left = 0
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        if target is not None:
            ...
            def show_action(gamestate):
                if target.turns_left > 0:
                    target.turns_left -= 1
                    if gamestate.time % 2 == 1:
                        return
                else:
                    target.action = lambda gs: Bee.action(target, gs)
                return Bee.action(target, gamestate)
            target.action = show_action
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        if self.place.ant is not self:
            # laserAnt has been contained
            insect_distance[self.place.ant] = distance
        if self.place.bees:
        ...
        while not place.is_hive:
            distance += 1
            if place.ant is not None:
                insect_distance[place.ant] = distance
                if place.ant.is_container and place.ant.ant_contained is not None:
                    insect_distance[place.ant.ant_contained] = distance
            if place.bees:
